...
Item {
    ...
    property ClockCircle cc: clockCircle
...
//                }
    ...
    Connections
    {
        ...
        onSendData:
        {
            ...
            if( Math.abs(parseFloat(tempSV))-0.2 <= Math.abs(parseFloat(tempPV)) && Math.abs(parseFloat(tempSV))+0.2 >= Math.abs(parseFloat(tempPV))  && busyIndicatorLayout.visible == true && label.text === qsTr("Ждем заданную температуру камеры "+temratureStayCamberText.text+"..."))
            {
            ...
            {
                if(busyIndicatorLayout.visible == true && label.text === qsTr("Имитатор запущен..."))
                {
