...
Item {
    ...
    ListModel {
        ...
        ListElement {
            name: "Проверка поиска, захвата и сопровождения сигнала с уровнем мощности минус 160 дБВт (уточнить так ли проводить)"
            ip: "5"
            connect: "Подробно"
        }
        ListElement {
        ...
        ListElement {
            name: "Проверка напряжения питания МШУ (нет в ПМ)"
            ip: "9"
            connect: "Подробно"
        }
        ListElement {
            name: "Проверка смещения секундной метки (нет в ПМ)"
            ip: "10"
            connect: "Подробно"
        }
                    ...
                    Button {
                        ...
                        onClicked:
                        {
                            ...
                            if(camber.slot_TestConnect())
                            {
                                ...
                                swipeView.visible = false; // поменять обратно на false
                                textCamberInform.visible = false;

                                tabBar.visible = false; // поменять обратно на false
    ...
    //             text: "В режиме \""+proverka.modeStart+"\" нельзя выполненять отдельные проверки."
